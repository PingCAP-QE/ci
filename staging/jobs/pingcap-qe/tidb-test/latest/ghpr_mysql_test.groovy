...
pipelineJob('pingcap-qe/tidb-test/ghpr_mysql_test') {
    // disabled(true)
    logRotator {
        daysToKeep(30)
    }
        ...
        pipelineTriggers {
            triggers {
                ghprbTrigger {
                    ...
                    triggerPhrase('.*/(debug-(all-tests|mysql-test).*)')
                    ...
                    extensions {
                        ...
                        ghprbSimpleStatus {
                            commitStatusContext("idc-jenkins-ci/mysql-test")
    ...
    definition {
        cpsScm {
            lightweight(true)
            scriptPath("staging/pipelines/PingCAP-QE/tidb-test/latest/ghpr_mysql_test.groovy")
            scm {
                git{
                    remote {
