...
pipelineJob('pingcap-qe/tidb-test/ghpr_build') {
    // disabled(true)
    logRotator {
        daysToKeep(30)
    }
        ...
        pipelineTriggers {
            triggers {
                ghprbTrigger {
                    ...
                    triggerPhrase('.*/(debug-(all-tests|build).*)')
    ...
    definition {
        cpsScm {
            lightweight(true)
            scriptPath("staging/pipelines/PingCAP-QE/tidb-test/latest/ghpr_build.groovy")
            scm {
                git{
                    remote {
