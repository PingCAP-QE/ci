#! /usr/bin/env bash
#
# DEPRECATED: This script is no longer actively used.
# Replaced by analyze-go-test-from-bazel-output.sh and the pipeline's
# built-in result reporting via cloudevents-server.
...
function report() {
    local jsonPath="$1"
    local fileServerUrl="${DEPRECATED_SERVER_URL:=http://fileserver.deprecated.net}"
    echo $fileServerUrl
