...
The directory convention is applied consistently in all three layers:

- Prow triggers: `prow-jobs/<org>/<repo>/<branch-special>-<jobtype>.yaml`
- Jenkins Job DSL: `jobs/<org>/<repo>/<branch-special>/<job>.groovy`
- Jenkins pipelines: `pipelines/<org>/<repo>/<branch-special>/<job>.groovy` and
  `pod-<job>.yaml`
...
its directory in all three layers; when it reaches end of life, remove it everywhere.
...
2. Create the branch directories and copy the closest existing release as a template:
   - `jobs/<org>/<repo>/release-X.Y/`
   - `pipelines/<org>/<repo>/release-X.Y/`
...
2. **Clean `PingCAP-QE/ci`** for every affected product component:
   ...
   - remove matching Jenkins Job DSL and pipeline resources,
