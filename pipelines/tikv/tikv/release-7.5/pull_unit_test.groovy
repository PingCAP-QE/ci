...
pipeline {
    ...
    stages {
        ...
        stage('build') {
            steps {
                retry(2) {
                    cache(path: "./${ARCHIVE_DIR}", includes: '**/*', key: prow.getCacheKey('ut-build', REFS)) {
                        dir(SRC_DIR) {
                            sh label: 'Build test artifact', script: """
                                if [ -f ${WORKSPACE}/${ARCHIVE_DIR}/${TEST_ARTIFACTS} ] && [ -f ${WORKSPACE}/${ARCHIVE_DIR}/${TEST_BINARIES_ARCHIVE} ]; then
                                    echo "Build cache hit: reuse ${ARCHIVE_DIR}, skip building test artifacts"
                                    exit 0
                                fi
