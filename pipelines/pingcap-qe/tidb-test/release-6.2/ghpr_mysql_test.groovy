...
final POD_TEMPLATE_FILE = 'pipelines/pingcap-qe/tidb-test/release-6.2/pod-ghpr_mysql_test.yaml'
    ...
    stages {
        ...
        stage('MySQL Tests') {
            matrix {
                ...
                stages {
                    stage("Test") {
                        when { expression { return !matrixCache.shouldSkip(REFS, 'Test', [part: env.PART, cache_enabled: env.CACHE_ENABLED]) } }
                        steps {
                            dir('tidb') {
                                cache(path: "./bin", includes: '**/*', key: "ws/${BUILD_TAG}/tidb-server") {
