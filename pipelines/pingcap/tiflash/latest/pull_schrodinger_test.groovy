        ...
        stage("Run") {
            steps {
                script {
                    ...
                    def runWithPod = { Closure body ->
                        def podCloud = "kubernetes-ng"
                        def podLabel = "${JOB_NAME}-${BUILD_NUMBER}"
                        def namespace = "jenkins-tiflash-schrodinger"
                        def podGoDockerImage = 'hub.pingcap.net/jenkins/centos7_golang-1.16:latest'
                        podTemplate(label: podLabel,
                                ...
                                containers: [
                                        containerTemplate(
                                                ...
                                                image: "${podGoDockerImage}", ttyEnabled: true,
