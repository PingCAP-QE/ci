...
def BUILD_URL = 'git@github.com:pingcap/tidb-lightning.git'

def build_path = 'go/src/github.com/pingcap/tidb-lightning'
def slackcolor = 'good'
def githash
def ws
...
try {
    node("${GO_BUILD_SLAVE}") {
                        ...
                        checkout changelog: false,
                                ...
                                scm: [$class: 'GitSCM',
                                        ...
                                        userRemoteConfigs: [[credentialsId: 'github-sre-bot-ssh',
                                                            ...
                                                            url: 'git@github.com:pingcap/tidb-lightning.git']]
                                ]
                    } else {
                        checkout scm: [$class: 'GitSCM', 
                            branches: [[name: branch]],  
                            extensions: [[$class: 'LocalBranch']],
                            userRemoteConfigs: [[credentialsId: 'github-sre-bot-ssh', url: 'git@github.com:pingcap/tidb-lightning.git']]]
                    }
                }
        ...
        stage("Upload") {
            dir(build_path) {
                def refspath = "refs/pingcap/tidb-lightning/${env.BRANCH_NAME}/sha1"
                def filepath = "builds/pingcap/tidb-lightning/${githash}/centos7/tidb-lightning.tar.gz"
                container("golang") {
                    timeout(10) {
                        sh """
                        echo "${githash}" > sha1
                        curl -F ${refspath}=@sha1 ${FILE_SERVER_URL}/upload
                        tar czvf tidb-lightning.tar.gz bin/*
                        curl -F ${filepath}=@tidb-lightning.tar.gz ${FILE_SERVER_URL}/upload
                        """
                    }
                }
            }
