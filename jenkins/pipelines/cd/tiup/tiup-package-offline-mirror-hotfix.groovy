...
def package_tools = { plat, arch ->
    ...
    importer_hash = ""
    if (VERSION >= "v5.2.0") {
        br_hash = sh(returnStdout: true, script: "python gethash.py -repo=tidb -version=${BR_TAG} -s=${FILE_SERVER_URL}").trim()
    } else {
    ...
    if(arch == "arm64") {
        ...
        sh """
            ...
            wget -qnc ${FILE_SERVER_URL}/download/builds/pingcap/br/optimization/${BR_TAG}/${br_hash}/centos7/br.tar.gz
            if [ $VERSION \\< "v5.2.0" ]; then
