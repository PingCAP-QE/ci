...
def check_image = { comps, edition, registry ->
    node(task) {
        ...
        comps.each {
            sh "python main.py image -c $it --registry ${registry} ${RELEASE_TAG}.json ${RELEASE_TAG} ${edition}"
        }
    }
}
...
def check_tiup = { comps, label ->
    node(label) {
        ...
        if (label == "mac" || label == "arm") {
            ...
            comps.each {
                sh "python main.py tiup -c $it ${RELEASE_TAG}.json ${RELEASE_TAG}"
            }
        }
    }
}
...
//            },
//            "Tiup Linux Amd64 [br,binlog,lightning,cdc,dumpling]": {
//                check_tiup(["br", "tidb-binlog", "tidb-lightning", "ticdc", "dumpling"], task)
//            },
