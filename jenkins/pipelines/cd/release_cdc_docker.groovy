...
if (!isNeedGo1160 && BUILD_TAG.startsWith("release-")) {
    isNeedGo1160 = isMoreRecentOrEqual(trimPrefix(BUILD_TAG), trimPrefix(releaseBranchUseGo1160))
    if (isNeedGo1160) {
        println "targetBranch=${BUILD_TAG}  >= ${releaseBranchUseGo1160}"
    }
}

if (BUILD_TAG.startsWith("v") && BUILD_TAG >= "v5.1.0") {
    isNeedGo1160 = true
}
      ...
      container('delivery') {
        ...
        stage("Build & Upload") {
          dir("go/src/github.com/pingcap/ticdc") {
            ...
            if ( DOCKER_TAG == "release-5.2" ){
                DOCKER_TAG = "v5.2.0-nightly"                                   
            }
            sh """
                cd /home/jenkins
                mkdir -p .docker
