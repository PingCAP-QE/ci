    ...
    node(loader) {
        container('toolkit') {
            withCredentials([string(credentialsId: 'ti-srebot', variable: 'TOKEN')]) {
                if (sh(script: "inv resolve-dir --path .ci --repo pingcap/tics --commit ${params.ghprbActualCommit} --auth ti-srebot:${TOKEN} && test -f integration_test.groovy", returnStatus: true) == 0) {
