...
def run_with_pod(Closure body) {
    ...
    if (GO_VERSION == "go1.20") {
        label = "tidb-ghpr-integration-common-test-go1120-${BUILD_NUMBER}"
    }
...
def run_with_memory_volume_pod(Closure body) {
    ...
    if (GO_VERSION == "go1.20") {
        label = "tidb-ghpr-integration-common-test-memory-volume-go1120-${BUILD_NUMBER}"
    }
    podTemplate(label: label,
            cloud: POD_CLOUD,
            namespace: POD_NAMESPACE,
