            ...
            tests["Integration MySQL Test"] = {
                try {
                    if (TIDB_TEST_BRANCH == "master") {
                        run("mysql_test", "mysqltest", "./test.sh alias alter_table alter_table1 alter_table_PK alter_table_partition analyze ansi auto_increment bigint binary bool boot_coll_server_binary bug28940878 bug33509 bug46760 bug58669 builtin bulk_replace case change_user charset check comment_column2 comment_table compare composite_index concurrent_ddl count_distinct count_distinct2 create_database create_index create_table ctype_gbk ctype_uca ctype_utf8 date_formats date_time_ddl datetime_insert datetime_update daylight_saving_time ddl_i18n_utf8 decimal default delete delete_myisam derived_cs desc_index_innodb do drop ds_mrr-big echo empty_table endspace error_simulation exec_selection expression_index field_length filesort filesort_json filesort_merge filter_single_col_idx_big func_bitwise_ops func_compress func_concat func_date_add_myisam func_default func_gconcat func_group_innodb func_group_innodb_16k func_if func_in_all func_in_none func_isnull func_math func_op func_prefix_key func_sapdb func_set func_str_myisam func_system func_test gcol_alter_table gcol_blocked_sql_funcs gcol_column_def_options gcol_dependenies_on_vcol gcol_illegal_expression gcol_ins_upd gcol_non_stored_columns gcol_partition gcol_select gcol_supported_sql_funcs gcol_view grant_dynamic grant_lowercase_fs group_min_max_ps_protocol groupby having heap_btree heap_btree_myisam histogram_singleton implicit_char_to_num_conversion in index index_merge1 index_merge2 index_merge_bug29952775 index_merge_bugs index_merge_delete index_merge_sqlgen_exprs index_merge_sqlgen_exprs_orandor_1_no_out_trans infoschema innodb_tmp_table_heap_to_disk insert insert_select insert_select_myisam insert_update invisible_indexes issue_11208 issue_165 issue_20571 issue_207 issue_227 issue_266 issue_294 join join-reorder join_crash join_outer json json_functions json_gcol key key_primary keywords lead_lag lead_lag_explain like limit limit_myisam long_tmpdir lowercase_fs_off lowercase_mixed_tmpdir lowercase_table lowercase_table2 lowercase_table4 lowercase_table5 lowercase_table_grant lowercase_utf8 lowercase_view lpad mariadb_cte_nonrecursive mariadb_cte_recursive math metadata multi_update multi_update_innodb multi_update_tiny_hash mysql_replace negation_elimination nth null_key_all_innodb odbc operator opt_hint_timeout opt_hints_index opt_hints_join_order opt_hints_subquery optimizer_bug12837084 order_by_limit order_by_sortkey orderby overflow packet_myisam parser parser_precedence partition partition_bug18198 partition_column_prune partition_grant partition_hash partition_innodb partition_list partition_locking_ps_protocol partition_order partition_range precedence prepare ps qualified regexp replace reset_connection role role2 roles-ddl roles-view roles2 roles_bugs_wildcard round row rowid_order_innodb rpad savepoint savepoint2 savepoint_in_optimistic_txn savepoint_in_pessimistic_txn savepoint_issue_26288 select_all select_qualified show show_check_cs show_check_cs_myisam show_profile show_variables single_delete_update sqllogic str_quoted strict strict_autoinc_2innodb sub_query sub_query_more subquery_antijoin subquery_bugs subquery_sj_innodb_all sum_distinct-big temp_table temptable_disk time  timestamp_insert timestamp_update tpcc transaction_isolation_func truth_value_transform type type_binary type_datetime type_decimal type_enum type_nchar type_set type_time type_timestamp type_timestamp_myisam type_uint type_unit type_varchar update update_myisam update_stmt user_if_exists user_var varbinary variable variables variables_dynamic_privs view_grant view_myisam warnings window_bitwise_ops window_functions window_functions_big window_functions_bugs window_functions_interesting_orders window_min_max with_non_recursive with_non_recursive_bugs with_recursive with_recursive_bugs xd  ")
                    } else {
