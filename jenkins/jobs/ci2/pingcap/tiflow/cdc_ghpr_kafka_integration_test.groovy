...
pipelineJob('cdc_ghpr_kafka_integration_test') {
    logRotator {
        daysToKeep(30)
    }
    parameters {
        ...
        pipelineTriggers {
            triggers {
                ghprbTrigger {
                    ...
                    triggerPhrase('(.*/run(-all-tests|-kafka-integration-test).*)|(/merge)')
                    ...
                    extensions {
                        ...
                        ghprbSimpleStatus {
                            commitStatusContext("idc-jenkins-ci-ticdc/kafka-integration-test")
    ...
    definition {
        cpsScm {
            lightweight(true)
            scriptPath("jenkins/pipelines/ci/ticdc/cdc_ghpr_kafka_integration_test.groovy")
            scm {
                git{
                    remote {
