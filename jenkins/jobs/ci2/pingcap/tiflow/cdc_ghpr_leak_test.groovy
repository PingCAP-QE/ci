...
pipelineJob('cdc_ghpr_leak_test') {
    disabled(true)
    logRotator {
        daysToKeep(30)
    }
        ...
        pipelineTriggers {
            triggers {
                ghprbTrigger {
                    ...
                    triggerPhrase('(.*/run(-all-tests|-leak-test).*)|(/merge)')
                    ...
                    extensions {
                        ...
                        ghprbSimpleStatus {
                            commitStatusContext("idc-jenkins-ci/leak-test")
    ...
    definition {
        cpsScm {
            lightweight(true)
            scriptPath("jenkins/pipelines/ci/ticdc/cdc_ghpr_leak_test.groovy")
            scm {
                git{
                    remote {
