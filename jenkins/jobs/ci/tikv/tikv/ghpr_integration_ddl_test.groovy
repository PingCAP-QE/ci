...
pipelineJob('tikv_ghpr_integration_ddl_test') {
    disabled(true)
    logRotator {
        daysToKeep(60)
        numToKeep(500)
        ...
        pipelineTriggers {
            triggers {
                ghprbTrigger {
                    ...
                    triggerPhrase('.*/run(-integration-ddl-test|-integration-tests).*')
                    ...
                    extensions {
                        ...
                        ghprbSimpleStatus {
                            commitStatusContext('idc-jenkins-ci-tikv/integration-ddl-test')
    ...
    definition {
        cpsScm {
            lightweight(true)
            scriptPath('jenkins/pipelines/ci/tikv/tikv_ghpr_integration_ddl_test.groovy')
            scm {
                git{
                    remote {
