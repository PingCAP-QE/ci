...
pipelineJob('tidb_test_ghpr_integration_test') {
    logRotator {
        daysToKeep(90)
        numToKeep(300)
    }
        ...
        pipelineTriggers {
            triggers {
                ghprbTrigger {
                    ...
                    triggerPhrase('.*/run(-integration-test).*')
                    ...
                    extensions {
                        ...
                        ghprbSimpleStatus {
                            commitStatusContext('idc-jenkins-ci/integration_test')
    ...
    definition {
        cpsScm {
            lightweight(true)
            scriptPath('jenkins/pipelines/ci/tidb-test/tidb_test_ghpr_integration_test.groovy')
            scm {
                git{
                    remote {
