...
pipelineJob('tidb_ghpr_tics_test') {
    logRotator {
        daysToKeep(30)
    }
    parameters {
        ...
        pipelineTriggers {
            triggers {
                ghprbTrigger {
                    ...
                    triggerPhrase('.*/(run(-integration-tests|-tics-test).*)')
                    ...
                    extensions {
                        ...
                        ghprbSimpleStatus {
                            commitStatusContext('idc-jenkins-ci-tidb/tics-test')
    ...
    definition {
        cpsScm {
            lightweight(true)
            scriptPath('jenkins/pipelines/ci/tidb/tidb_ghpr_tics_test.groovy')
            scm {
                git{
                    remote {
