...
pipelineJob('tidb_ghpr_integration_ddl_test') {
    logRotator {
        daysToKeep(30)
    }
    parameters {
        ...
        pipelineTriggers {
            triggers {
                ghprbTrigger {
                    ...
                    triggerPhrase('.*/(run(-integration-tests|-integration-ddl-test).*)')
                    ...
                    extensions {
                        ...
                        ghprbSimpleStatus {
                            commitStatusContext('idc-jenkins-ci-tidb/integration-ddl-test')
    ...
    definition {
        cpsScm {
            lightweight(true)
            scriptPath('jenkins/pipelines/ci/tidb/tidb_ghpr_integration_ddl_test.groovy')
            scm {
                git{
                    remote {
