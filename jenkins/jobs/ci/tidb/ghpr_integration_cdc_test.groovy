...
pipelineJob('tidb_ghpr_integration_cdc_test') {
    logRotator {
        daysToKeep(30)
    }
    parameters {
        ...
        pipelineTriggers {
            triggers {
                ghprbTrigger {
                    ...
                    triggerPhrase('.*/(run(-integration-tests|-integration-cdc-test).*)')
                    ...
                    extensions {
                        ...
                        ghprbSimpleStatus {
                            commitStatusContext('idc-jenkins-ci-tidb/integration-cdc-test')
    ...
    definition {
        cpsScm {
            lightweight(true)
            scriptPath('jenkins/pipelines/ci/tidb/tidb_ghpr_integration_cdc_test.groovy')
            scm {
                git{
                    remote {
