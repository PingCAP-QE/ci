...
pipelineJob('tidb_ghpr_common_test') {
    disabled(true)
    logRotator {
        daysToKeep(90)
    }
        ...
        pipelineTriggers {
            triggers {
                ghprbTrigger {
                    ...
                    triggerPhrase('.*/(run(-integration-tests|-common-test).*)')
                    ...
                    extensions {
                        ...
                        ghprbSimpleStatus {
                            commitStatusContext('idc-jenkins-ci-tidb/common-test')
    ...
    definition {
        cpsScm {
            lightweight(true)
            scriptPath('jenkins/pipelines/ci/tidb/tidb_ghpr_common_test.groovy')
            scm {
                git{
                    remote {
