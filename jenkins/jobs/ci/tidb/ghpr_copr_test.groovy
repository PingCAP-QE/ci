...
pipelineJob('tidb_ghpr_integration_copr_test') {
    logRotator {
        daysToKeep(30)
    }
    parameters {
        ...
        pipelineTriggers {
            triggers {
                ghprbTrigger {
                    ...
                    triggerPhrase('.*/(run(-integration-tests|-integration-copr-test).*)')
                    ...
                    extensions {
                        ...
                        ghprbSimpleStatus {
                            commitStatusContext('idc-jenkins-ci-tidb/integration-copr-test')
    ...
    definition {
        cpsScm {
            lightweight(true)
            scriptPath('jenkins/pipelines/ci/tidb/tidb_ghpr_integration_copr_test.groovy')
            scm {
                git{
                    remote {
